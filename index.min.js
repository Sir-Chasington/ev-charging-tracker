const chargeHistory=JSON.parse(localStorage.getItem("chargeHistory"))||[];let totalCost=chargeHistory.reduce((sum,entry)=>sum+entry.cost,0);function updateOldEntries(){let updated=false;chargeHistory.forEach(entry=>{if(entry.percentCharged===undefined){entry.percentCharged="-";updated=true}});if(updated){localStorage.setItem("chargeHistory",JSON.stringify(chargeHistory))}}function initializeCarMake(){const carMake=localStorage.getItem("carMake");const carMakeInput=document.getElementById("carMake");const carMakeDisplay=document.getElementById("carMakeDisplay");if(carMake){carMakeInput.style.display="none";carMakeDisplay.style.display="inline";carMakeDisplay.textContent=carMake}else{carMakeInput.style.display="inline";carMakeDisplay.style.display="none"}}function saveCarMake(){const carMakeInput=document.getElementById("carMake");const carMake=carMakeInput.value.trim();if(carMake){localStorage.setItem("carMake",carMake);initializeCarMake()}}function toggleEndPercentage(){const chargeToFull=document.getElementById("chargeToFull").checked;const endPercentageContainer=document.getElementById("endPercentageContainer");if(chargeToFull){endPercentageContainer.style.display="none"}else{endPercentageContainer.style.display="block"}}function renderHistory(){const tableBody=document.getElementById("chargeHistory");tableBody.innerHTML="";chargeHistory.forEach((entry,index)=>{const percentCharged=entry.percentCharged!==undefined&&entry.percentCharged!=="Unknown"?`${entry.percentCharged}% charged`:"-";const row=document.createElement("tr");row.innerHTML=`
      <td>${entry.date}</td>
